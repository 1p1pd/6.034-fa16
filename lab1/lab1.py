...
F0 = IF( 'person (?x)',
    THEN('self (?x) (?x)'))
F1 = IF( AND('parent (?x) (?y)',
            'parent (?x) (?z)',
            NOT('self (?y) (?z)')),
    THEN('sibling (?y) (?z)',
        'sibling (?z) (?y)'))
F2 = IF( AND('sibling (?x) (?y)',
            'parent (?x) (?a)',
            'parent (?y) (?b)',
            NOT('sibling (?a) (?b)')),
    THEN('cousin (?a) (?b)',
        'cousin (?b) (?a)'))
F3 = IF( AND('parent (?x) (?y)',
            'parent (?y) (?z)'),
    THEN('grandparent (?x) (?z)'))
F4 = IF( 'grandparent (?x) (?y)',
    THEN('grandchild (?y) (?x)'))
F5 = IF( 'parent (?x) (?y)',
    THEN('child (?y) (?x)'))

# Add your rules to this list:
family_rules = [F0, F1, F2, F3, F4, F5]
...
def backchain_to_goal_tree(rules, hypothesis):
    ...
    for rule in rules:
        for cons in rule.consequent():
            dic = match(cons, hypothesis)
            if dic != None:
                if isinstance(rule.antecedent(), AND):
                    temp = AND()
                    for preq in rule.antecedent():
                        temp.append(backchain_to_goal_tree(rules, populate(preq, dic)))
                    result.append(temp)
                elif isinstance(rule.antecedent(), str):
                    result.append(backchain_to_goal_tree(rules, populate(rule.antecedent(), dic)))
                else:
                    for preq in rule.antecedent():
                        result.append(backchain_to_goal_tree(rules, populate(preq, dic)))
